...
#define DEVICE_STRUCT_SIZE 0x57  // Size per device structure (from Ghidra analysis)
    ...
    BOOL ExtractEnhancedDeviceCapabilities(
        void* extensionBuffer,
        HANDLE driveHandle,
...
private:
    ...
    BOOL fetchInquiryData(
        HANDLE hDevice,
        BYTE volumeIndex,
        LPVOID inquiryBuffer);                                // Added for STD_Inquiry
    BOOL readCapacity(BYTE volumeIndex, LPDWORD lpCapacity);  // Added for STD_ReadCapacity
    BOOL logicalRead(
        BYTE volumeIndex,
        LPVOID lpBuffer,
        DWORD lba,
        DWORD sectors);  // Added for STD_LogicalRead
