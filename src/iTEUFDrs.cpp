...
BOOL iTEUFDrs::DetectLogicalVolumes() {
    ...
    for(BYTE idx = 0; idx < m_deviceInfo.volumeCount; ++idx) {
        ...
        if(s.find("1181") != std::string::npos) {
            v.familyType = 0x1181;
            v.controllerType = 0;
            if(s.find("A0AA") != std::string::npos)
                v.a1baFlag = 0;
            else if(s.find("A1BA") != std::string::npos) {
                v.controllerType = 1;
                v.a1baFlag = 1;
            }
...
BOOL iTEUFDrs::fetchInquiryData(BYTE volumeIndex, BYTE* outBuffer, DWORD bufferSize) {
    if(volumeIndex >= m_deviceInfo.volumeCount || ! outBuffer || bufferSize < 0xB0)
        return FALSE;
...
BOOL iTEUFDrs::ProcessDeviceInquiry(HANDLE hDevice, BYTE volumeIndex) {
    ...
    if(FLH_GetInfoFromDataBaseByID) {
        ...
        if(result) {
            ...
            memcpy(m_deviceInfo.volumes[volumeIndex].vendorName, inquiryBuffer + 8, 8);
            memcpy(m_deviceInfo.volumes[volumeIndex].productName, inquiryBuffer + 16, 16);
...
void iTEUFDrs::VolumePairController() {
    ...
    for(BYTE volumeIndex = 0;
        volumeIndex < m_deviceInfo.volumeCount && m_controllerCount < MAX_CONTROLLERS;
        volumeIndex++) {
        ...
        BYTE pairCount = 1;
        for(BYTE otherIndex = 0; otherIndex < m_deviceInfo.volumeCount && pairCount < 4;
            otherIndex++) {
            ...
            if(otherVolume.deviceFound && otherVolume.volumeLetter == baseDriveLetter) {
                ...
                LogMessage(
                    "VolumePairController: Paired volume %d with base volume %d (drive %c)",
                    otherIndex,
                    volumeIndex,
                    baseDriveLetter);
        ...
        if(! ValidateControllerConfiguration(&controller)) {
            LogWarning(
                "VolumePairController: Controller %d configuration validation failed",
                m_controllerCount);
        ...
        LogMessage(
            "VolumePairController: Controller %d configured with %d volumes (Type: %d, Product: "
            "0x%08X)",
            m_controllerCount,
    ...
    LogMessage(
        "VolumePairController: Configured %d controllers from %d volumes",
        m_controllerCount,
        m_deviceInfo.volumeCount);
}
...
BOOL iTEUFDrs::ValidateControllerConfiguration(CONTROLLER_DATA* controller) {
    ...
    if(controller->volumeCount == 0 || controller->volumeCount > 4) {
        LogError(
            "ValidateControllerConfiguration: Invalid volume count: %d", controller->volumeCount);
        return FALSE;
    }
    ...
    if(controller->controllerType > 0x10) {
        LogWarning(
            "ValidateControllerConfiguration: Unusual controller type: 0x%02X",
            controller->controllerType);
    }
    ...
    LogMessage(
        "ValidateControllerConfiguration: Controller validation passed - %d volumes, type 0x%02X",
        controller->volumeCount,
        controller->controllerType);
    return TRUE;
}
...
BOOL iTEUFDrs::CheckSystemReadyIO(BYTE volumeIndex, DWORD deviceId) {
    LogMessage(
        "iTEUFDrs: Checking system ready IO for volume %d, device ID %lu", volumeIndex, deviceId);
...
BOOL iTEUFDrs::LoadBankC2(BYTE volumeIndex, DWORD deviceId) {
    return TRUE;
}
BOOL iTEUFDrs::GetBCMInformation2(BYTE volumeIndex, DWORD deviceId) {
    return TRUE;
}
...
BOOL iTEUFDrs::GetMPInfo(BYTE volumeIndex, DWORD deviceId) {
    ...
    if(readResult != 0) {
        ...
        LogMessage(
            "GetMPInfo: Successfully loaded MP info for volume %d. Version: %d.%d",
            volumeIndex,
            volume.mpInfo.majorVersion,
            volume.mpInfo.minorVersion);
        success = TRUE;
    } else {
...
BOOL iTEUFDrs::GetLunArrayData(BYTE volumeIndex, DWORD deviceId) {
    ...
    if(pReadLUNArray) {
        ...
        if(readResult == 0) {
            ...
            result = TRUE;
            LogMessage(
                "GetLunArrayData: Successfully read LUN array data for volume %d", volumeIndex);
        } else {
...
void iTEUFDrs::CalculateCapacity(BYTE volumeIndex) {
    ...
    if(readCapacity(volumeIndex, &lbaMax, &blkSize)) {
        ...
        LogMessage(
            "CalculateCapacity: Calculated capacity for volume %d: %lu bytes",
            volumeIndex,
            volume.capacity);
    } else {
...
void iTEUFDrs::CalculateRealCapacity(BYTE volumeIndex) {
    ...
    LogMessage(
        "CalculateRealCapacity: Calculated real capacity for volume %d: %lu bytes",
        volumeIndex,
        volume.realCapacity);
}
...
BOOL iTEUFDrs::setupDatabasePaths() {
    ...
    if(fdb[0] != '\0') {
        HANDLE h = CreateFileA(
            fdb, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
        if(h != INVALID_HANDLE_VALUE) {
            CloseHandle(h);
            return TRUE;
        }
    ...
    if(cdb[0] != '\0') {
        HANDLE h = CreateFileA(
            cdb, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
        if(h != INVALID_HANDLE_VALUE) {
            CloseHandle(h);
            return TRUE;
        }
...
BOOL iTEUFDrs::RunInitializationChain() {
    ...
    for(BYTE ci = 0; ci < controllerCount; ++ci) {
        ...
        BOOL opened =
            m_forcedMode ? OpenDriveHandleAgain(volIndex) : CheckDriveExistInternal(volIndex);
        if(! opened) {
...
typedef int(__stdcall* PFN_FLH_SCAN_MASS_BLOCKS_PER_CHIP)(
    ...
    int* outRet);
typedef int(
    __stdcall* PFN_FLH_GET_SCAN_RESULT)(DWORD ctx, BYTE ce, BYTE ch, int rtPtr, void* outBuf);
typedef int(__stdcall* PFN_FLH_READ_SPARE)(
    BYTE ch,
    BYTE ce,
    int rtPtr,
    void* spare,
...
static inline bool testBit(byte mask, byte bit) {
    return ((mask >> (bit & 7)) & 1) == 1;
}

BOOL iTEUFDrs::scanMassBlocks(BYTE mode) {
    LOG_INFO("scanMassBlocks: mode=%u", (unsigned) mode);
    if(m_deviceInfo.selectedVolume >= m_deviceInfo.volumeCount)
        return FALSE;
    ...
    PFN_FLH_SCAN_MASS_BLOCKS_PER_CHIP pScan =
        (PFN_FLH_SCAN_MASS_BLOCKS_PER_CHIP) g_VDR_MassBlocksProcess;
    PFN_FLH_GET_SCAN_RESULT pGetScan =
        (PFN_FLH_GET_SCAN_RESULT) g_FLH_GetChannelCeNoAndMap;  // placeholder mapping
    ...
    for(BYTE ce = 0; ce < 8; ++ce) {
        if(! testBit(ceMask, ce))
            continue;
        for(BYTE ch = 0; ch < 2; ++ch) {
            if(! testBit(chMask, ch))
                continue;
            LOG_DEBUG("scanMassBlocks: CE=%u CH=%u", ce, ch);
            if(pScan) {
                int ok = pScan(0, ce, ch, 0, tmpBuf, mode, &outFlag, &outRet);
                if(! ok)
                    LOG_WARNING("flhScanMassBlocksPerChip failed: ce=%u ch=%u", ce, ch);
            }
            if(pGetScan) {
                int ok2 = pGetScan(0, ce, ch, 0, tmpBuf);
                if(! ok2)
                    LOG_WARNING("flhGetScanResult failed: ce=%u ch=%u", ce, ch);
            }
            if(pReadSpare) {
...
BOOL iTEUFDrs::flhScanMassBlocksPerChip(
    DWORD ctx,
    BYTE ce,
    BYTE ch,
    int rtPtr,
    ...
    BYTE* outFlag,
    int* outRet) {
    PFN_FLH_SCAN_MASS_BLOCKS_PER_CHIP p =
        (PFN_FLH_SCAN_MASS_BLOCKS_PER_CHIP) g_VDR_MassBlocksProcess;
    if(! p) {
        LOG_WARNING("g_VDR_MassBlocksProcess not bound");
        return FALSE;
    }
...
BOOL iTEUFDrs::flhGetScanResult(DWORD ctx, BYTE ce, BYTE ch, int rtPtr, void* outBuf) {
    ...
    if(! p) {
        LOG_WARNING("g_FLH_GetChannelCeNoAndMap not bound");
        return FALSE;
    }
...
BOOL iTEUFDrs::logicalRead(BYTE volumeIndex, DWORD lba, BYTE* outBuffer, DWORD bytes) {
    if(volumeIndex >= m_deviceInfo.volumeCount || ! outBuffer || bytes == 0)
        return FALSE;
    DEVICE_VOLUME_INFO& v = m_deviceInfo.volumes[volumeIndex];
    ...
    if(pLogRead) {
        ...
        if(! ok) {
            LOG_WARNING(
                "STD_LogicalRead failed for %s lba=%lu bytes=%lu",
                path,
                (unsigned long) lba,
                (unsigned long) bytes);
        }
...
typedef int(__stdcall* PFN_FLH_READSPARE)(
    DWORD ch,
    DWORD ce,
    DWORD block,
    DWORD page,
...
BOOL iTEUFDrs::checkSystemReady(BYTE volumeIndex) {
    ...
    DEVICE_VOLUME_INFO& v = m_deviceInfo.volumes[volumeIndex];
    char path[8];
    buildVolumePath((char) v.volumeLetter, path);
...
BOOL iTEUFDrs::readBCM(BYTE volumeIndex, BYTE* outBuf, DWORD bufSize) {
    if(volumeIndex >= m_deviceInfo.volumeCount || ! outBuf || bufSize == 0)
        return FALSE;
    DEVICE_VOLUME_INFO& v = m_deviceInfo.volumes[volumeIndex];
    char path[8];
    buildVolumePath((char) v.volumeLetter, path);
    ...
    if(pReadBCM) {
        ...
        if(! ok)
            LOG_WARNING("FLH_ReadBCM failed for %s", path);
    } else {
...
BOOL iTEUFDrs::readSpare(
    ...
    if(pReadSpare) {
        ...
        if(! ok)
            LOG_WARNING(
                "FLH_ReadSpare failed ch=%lu ce=%lu blk=%lu pg=%lu",
                (unsigned long) ch,
                (unsigned long) ce,
                (unsigned long) block,
                (unsigned long) page);
...
BOOL iTEUFDrs::blockErase(BYTE volumeIndex, DWORD ch, DWORD ce, DWORD block) {
    ...
    if(pErase) {
        ...
        if(! ok)
            LOG_WARNING(
                "FLH_BlockErase failed ch=%lu ce=%lu blk=%lu",
                (unsigned long) ch,
                (unsigned long) ce,
                (unsigned long) block);
    } else {
...
BOOL iTEUFDrs::cpuReset(BYTE volumeIndex) {
    ...
    if(pCpuReset) {
        ok = (pCpuReset(h) != 0);
        if(! ok)
            LOG_WARNING("FLH_CPUReset failed for %s", path);
    } else {
        LOG_WARNING("g_FLH_CPUReset not bound");
    }
    CloseHandle(h);
...
BOOL iTEUFDrs::vdrReset(BYTE volumeIndex) {
    ...
    if(pFRst) {
        ok = (pFRst(h) != 0);
        if(! ok)
            LOG_WARNING("VDR_F_RST failed for %s", path);
    } else {
    ...
    if(bytesRead == sizeof(versionData)) {
        ...
        if(iteuSignature && (iteuSignature - (char*) versionData) != -1) {
            ...
            if(sprintf_s(
                   ...
                   sizeof(bank.versionString),
                   " %s%s",
                   (char*) &versionData[0x10],
                   (char*) &versionData[0x14])
               <= 0) {
...
BOOL iTEUFDrs::NotifyFwSegmentInfo(BYTE volumeIndex, DWORD deviceId) {
    LogMessage(
        "NotifyFwSegmentInfo: Notifying for volume %d, device ID %lu", volumeIndex, deviceId);
    ...
    // int arrangeResult = m_sdkApis.FLH_ArrangeSegmentPara(segmentParams,
    // &volume.banks[0].bankData[0x1866]);

    // Since we don't have the data, we'll log a warning and proceed as if it succeeded for now.
    LogWarning(
        "NotifyFwSegmentInfo: Skipping FLH_ArrangeSegmentPara as bank data is not yet loaded.");
    ...
    if(arrangeResult == 1) {
        ...
        // The call would look like this:
        // int initResult = m_sdkApis.FLH_InitCTRL(volume.hDevice, segmentParams,
        // &volume.bcmInfo[0xA26]);
        ...
        if(initResult == 1) {
            ...
            LogMessage(
                "NotifyFwSegmentInfo: Successfully notified (simulated) for volume %d",
                volumeIndex);
            return TRUE;
        } else {
    ...
    for(BYTE ce = 0; ce < 8; ce++) {
        for(BYTE ch = 0; ch < 2; ch++) {
            ...
            if(g_FLH_HandleMassBlocksPerChip) {
                ...
                int scanResult = ((
                    int(__stdcall*)(DWORD, BYTE, BYTE, int, void*, BYTE, BYTE*, int*))
                                      g_FLH_HandleMassBlocksPerChip)(
                    deviceId, ce, ch, (int) &volume.banks[0], scanBuffer, mode, &outFlag, &outRet);
...
UINT iTEUFDrs::EnhancedOpenDriveHandleAgain() {
    ...
    const char* physicalDrivePaths[] = { "\\\\.\\PhysicalDrive1", "\\\\.\\PhysicalDrive2",
                                         "\\\\.\\PhysicalDrive3", "\\\\.\\PhysicalDrive4",
                                         "\\\\.\\PhysicalDrive5", "\\\\.\\PhysicalDrive6",
                                         "\\\\.\\PhysicalDrive7", "\\\\.\\PhysicalDrive8" };
    ...
    __try {
        ...
        for(DWORD driveIndex = 0; driveIndex < 8; driveIndex++) {
            ...
            if(! PerformSecureDeviceInquiry(inquiryBuffer, driveHandle, driveIndex)) {
                LogError(
                    "EnhancedOpenDriveHandleAgain: Device inquiry failed for drive %d", driveIndex);
                CloseHandle(driveHandle);
                continue;
            }

            // Enhanced device analysis and validation
            if(! AnalyzeAndValidateDevice(
                   inquiryBuffer, extensionBuffer, driveHandle, driveIndex, structOffset)) {
                LogMessage(
                    "EnhancedOpenDriveHandleAgain: Device validation failed for drive %d",
                    driveIndex);
                CloseHandle(driveHandle);
                continue;
            }
            ...
            foundDevices++;
            LogMessage(
                "EnhancedOpenDriveHandleAgain: Successfully detected device %d", foundDevices);
        ...
        LogMessage(
            "EnhancedOpenDriveHandleAgain: Detection complete. Found %d devices", foundDevices);

    } __except(EXCEPTION_EXECUTE_HANDLER) {
        LogError(
            "EnhancedOpenDriveHandleAgain: Exception during device scanning - 0x%08X",
            GetExceptionCode());
        foundDevices = 0;
    }
...
BOOL iTEUFDrs::PerformSecureDeviceInquiry(
    void* inquiryBuffer,
    HANDLE driveHandle,
    DWORD driveIndex) {
    ...
    if(! result) {
        DWORD error = GetLastError();
        LogError(
            "PerformSecureDeviceInquiry: SDK inquiry failed for drive %d, error: %d",
            driveIndex,
            error);
        return FALSE;
    }
...
BOOL iTEUFDrs::AnalyzeAndValidateDevice(
    void* inquiryBuffer,
    ...
    LogMessage(
        "AnalyzeAndValidateDevice: Device %d validated - Type: 0x%04X, FW: %d, Rev: %d",
        driveIndex,
        deviceInfo.controllerType,
        deviceInfo.firmwareType,
        deviceInfo.revisionType);
...
BOOL iTEUFDrs::ExtractEnhancedDeviceCapabilities(
    void* extensionBuffer,
    HANDLE driveHandle,
...
BOOL iTEUFDrs::SecureMemoryAllocation(void** buffer, SIZE_T size, const char* purpose) {
    ...
    *buffer = HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, size);
    if(! *buffer) {
        LogError(
            "SecureMemoryAllocation: Failed to allocate %zu bytes for %s",
            size,
...
void iTEUFDrs::UpdateDeviceStatusFromScan(BYTE volumeIndex, BYTE ce, BYTE ch, BYTE* scanData) {
    if(volumeIndex >= m_deviceInfo.volumeCount || ! scanData)
        return;
    ...
    if(g_FLH_BlockErase) {
        for(DWORD block = 0; block < volume.blockCount; block++) {
            if(volume.blockMap[ce][ch][block] == 0xFF) {  // Bad block marker
                int eraseResult =
                    ((int(__stdcall*)(DWORD, DWORD, BYTE, BYTE, HANDLE)) g_FLH_BlockErase)(
                        deviceId, block, ce, ch, volume.hDevice);
                if(eraseResult != 1) {
...
BOOL iTEUFDrs::GetDeviceInfoMain(BYTE volumeIndex, DWORD deviceId) {
    ...
    for(BYTE bankIndex = 0; bankIndex < MAX_BANKS; bankIndex++) {
        ...
        if(g_FLH_ReadBCM) {
            int result =
                ((int(__stdcall*)(void*, HANDLE)) g_FLH_ReadBCM)(&bank.bcmInfo, volume.hDevice);
            if(result != 1) {
                switch(result) {
                case 0: MessageBoxA(NULL, "Get BCM information CMD fail", "Error", MB_OK); break;
                case 0x3F: MessageBoxA(NULL, "Get BCM information IO fail", "Error", MB_OK); break;
                case 0x72: MessageBoxA(NULL, "Get BCM information fail", "Error", MB_OK); break;
                case 0x74:
                    MessageBoxA(
                        NULL, "Notify fw to park at runtime BCM information fail", "Error", MB_OK);
                    break;
                }
                goto cleanup;
            }
        ...
        FormatDeviceString(
            ...
            sizeof(m_deviceInfo.deviceString),
            " %s%s , ( %C )\\n%s",
            selectedVolume.vendorName,
            selectedVolume.productName,
            selectedVolume.volumeLetter,
        ...
        for(BYTE i = 1; i < 4; i++) {
            ...
            CHAR paramStr[0x100];
            FormatDeviceString(
                paramStr, sizeof(paramStr), "( %C )", m_deviceInfo.volumes[param].volumeLetter);
...
BOOL iTEUFDrs::GetFlashMethod(BYTE volumeIndex, DWORD deviceId) {
    ...
    if(g_FLH_GetFlashDataFromDataBase) {
        BOOL result =
            ((BOOL(__stdcall*)(DWORD, BYTE*, BYTE*, CHAR*)) g_FLH_GetFlashDataFromDataBase)(
                deviceId, flashData, m_deviceInfo.deviceData, m_deviceInfo.devicePath);
        ...
        if(! volume.systemReady) {
            ...
            if(g_FLH_GetFlashDataFromMemory) {
                BOOL memoryResult = ((BOOL(__stdcall*)(DWORD, BYTE*)) g_FLH_GetFlashDataFromMemory)(
                    deviceId, flashData);
...
void iTEUFDrs::UpdateFlagsAfterFlashParse(BYTE volumeIndex) {
    ...
    auto readByte = [&](size_t off) -> BYTE {
        if(off < sizeof(bank.bcmInfo))
            return bank.bcmInfo[off];
        return 0;
    };
...
BOOL iTEUFDrs::CheckNeedLoadBank(BYTE volumeIndex, DWORD deviceId) {
    ...
    if(g_VDR_ReadWriteLUNConfig) {
        int result = ((int(__stdcall*)(DWORD, DWORD*, BYTE*, DWORD)) g_VDR_ReadWriteLUNConfig)(
            0, lunConfig, &bank.bcmInfo[0xA26], deviceId);
...
BOOL iTEUFDrs::LoadBankData2(BYTE volumeIndex, DWORD deviceId) {
    ...
    if(g_FLH_FindRootTable) {
        BYTE result = ((BYTE(__stdcall*)(DWORD, DWORD*, BYTE*, DWORD)) g_FLH_FindRootTable)(
            deviceId, rootTableEntries, &bank.bcmInfo[0xA26], 0);
        if(result != 0) {
    ...
    if(rootTableCount != 0) {
        for(DWORD i = 0; i < rootTableCount; i++) {
            ...
            if(g_VDR_RootFunc) {
                int readResult = ((int(__stdcall*)(
                    DWORD, DWORD, DWORD, DWORD, DWORD, BYTE*, BYTE*, DWORD)) g_VDR_RootFunc)(
                    rootTableAddresses[i],
                    1,
                    0x40,
                    1,
                    0x200,
                ...
                if(readResult == 1) {
                    ...
                    for(int j = 0; j < 2; j++) {
                        ...
                        DWORD rootTableEntry =
                            ProcessRootTableEntry(rootTablePtr[j], deviceId, &bank.bcmInfo[0xA26]);
                        ...
                        DWORD convertedAddress =
                            ConvertBlockAddress(&bank.bcmInfo[0xA26], rootTableWord);
...
BOOL iTEUFDrs::LoadBankData3(BYTE volumeIndex, DWORD deviceId) {
    ...
    if(g_VDR_ReadSysAddr) {
        int readResult = ((int(__stdcall*)(DWORD*, BYTE*, DWORD)) g_VDR_ReadSysAddr)(
            sysAddrData, &bank.bcmInfo[0xA26], deviceId);
        if(readResult != 0) {
            // Process system address data
            for(int i = 0; i < 2; i++) {
                // Get system address entry
                DWORD sysAddrEntry =
                    ProcessRootTableEntry(sysAddrData[i], deviceId, &bank.bcmInfo[0xA26]);
                DWORD sysAddrEntry2 = sysAddrData[i + 2];

                // Store system address data
                bank.sysAddrData[i] = sysAddrEntry;
                bank.sysAddrData[i + 2] =
                    ProcessRootTableEntry(sysAddrEntry2, deviceId, &bank.bcmInfo[0xA26]);
...
UINT iTEUFDrs::OpenDriveHandleAgain(int param_1) {
    // EXACT GHIDRA MCP DECOMPILATION - OpenDriveHandleAgain at 0x0040beb0
    // This is the precise implementation from the original binary

    uint local_2c = 0;  // Detected devices counter

    // Get manager instance via FUN_0041fe69 call
    int* piVar2 = (int*) GetManager();
    if(piVar2 == nullptr) {
        piVar2 = (int*) ErrorHandler::ReportError(0x80004005);
    }

    // Get controller data base address (iVar3 + 0x10)
    int iVar3 = GetControllerDataBase();
    uchar* _Str = (uchar*) (iVar3 + 0x10);

    // Physical drive paths array - EXACT from decompilation
    const char* physicalDrivePaths[8] = { "\\\\.\\PhysicalDrive1", "\\\\.\\PhysicalDrive2",
                                          "\\\\.\\PhysicalDrive3", "\\\\.\\PhysicalDrive4",
                                          "\\\\.\\PhysicalDrive5", "\\\\.\\PhysicalDrive6",
                                          "\\\\.\\PhysicalDrive7", "\\\\.\\PhysicalDrive8" };

    // Allocate buffers - EXACT sizes from decompilation
    void* local_1c = malloc(0xB0);   // Inquiry buffer
    void* local_28 = malloc(0xE40);  // Extended data buffer
    ...
    int local_24 = 0;  // Drive index loop variable

    // Main drive scanning loop - drives 0-7 (PhysicalDrive1-8)
    while(local_24 < 8) {
        // Calculate structure offset (0x57 bytes per device)
        int iVar3_offset = local_2c * 0x57;

        // Initialize device structure at offset 0x62a2
        *(char*) (iVar3_offset + 0x62a2 + param_1) = (char) local_24;

        // Initialize LUN and Target IDs to 0xFF
        *(unsigned char*) (iVar3_offset + 0x62e1 + param_1) = 0xFF;  // LUN ID
        *(unsigned char*) (iVar3_offset + 0x62e0 + param_1) = 0xFF;  // Target ID

        // Try to open physical drive using FUN_00408580
        char openResult = OpenPhysicalDriveByIndex(local_2c);
        if(openResult == '\0') {
            local_24++;
            continue;
        }

        // Get device handle from structure offset 0x62a3
        HANDLE local_20 = *(HANDLE*) (iVar3_offset + 0x62a3 + param_1);

        // Clear inquiry buffer
        memset(local_1c, 0, 0xB0);

        // Call SDK inquiry function - DAT_004ad520 function pointer
        int inquiryResult = CallSDKInquiry(local_1c, local_20);
        if(inquiryResult == 0) {
            LogMessage("Vol = %d, Can't get inquiry data ...", local_24);
            CloseHandle(local_20);
            local_24++;
            continue;
        }

        // Format inquiry string from offset 0x24 in inquiry buffer
        FormatInquiryString(&_Str, (int) local_1c + 0x24);
        LogMessage("Vol = %d, Inquiry = %s", local_24, _Str);

        // Check for "ITEu" signature in inquiry string
        uchar* iteSignature = FindSubstring(_Str, "ITEu");
        if(iteSignature == nullptr || ((int) iteSignature - (int) _Str) == -1) {
            LogMessage("It's not our device ...");
            CloseHandle(local_20);
            local_24++;
            continue;
        }

        // Initialize controller type to unsupported (200)
        *(unsigned char*) (iVar3_offset + 0x62f6 + param_1) = 200;

        // Check for ITE 1181 controller
        uchar* ite1181 = FindSubstring(_Str, "1181");
        if(ite1181 != nullptr && ((int) ite1181 - (int) _Str) != -1) {
            *(unsigned short*) (iVar3_offset + 0x62f4 + param_1) = 0x1181;  // Product ID
            *(unsigned char*) (iVar3_offset + 0x62f6 + param_1) = 0;        // Controller type
            *(unsigned char*) (iVar3_offset + 0x62f8 + param_1) = 0xFF;     // Revision

            // Check for A0AA revision
            uchar* revA0AA = FindSubstring(_Str, "A0AA");
            if(revA0AA != nullptr && ((int) revA0AA - (int) _Str) != -1) {
                *(unsigned char*) (iVar3_offset + 0x62f8 + param_1) = 0;
            } else {
                // Check for A1BA revision
                uchar* revA1BA = FindSubstring(_Str, "A1BA");
                if(revA1BA != nullptr && ((int) revA1BA - (int) _Str) != -1) {
                    *(unsigned char*) (iVar3_offset + 0x62f6 + param_1) = 1;  // Controller type 1
                    *(unsigned char*) (iVar3_offset + 0x62f8 + param_1) = 1;  // Revision 1
                }
            }
        } else {
            // Check for ITE 1176 controller
            uchar* ite1176 = FindSubstring(_Str, "1176");
            if(ite1176 != nullptr && ((int) ite1176 - (int) _Str) != -1) {
                *(unsigned short*) (iVar3_offset + 0x62f4 + param_1) = 0x1176;  // Product ID
                *(unsigned char*) (iVar3_offset + 0x62f6 + param_1) = 2;        // Controller type
                *(unsigned char*) (iVar3_offset + 0x62f8 + param_1) = 0xFF;     // Revision
        ...
        // Check if device type is still unsupported (200 = 0xC8)
        if(*(char*) (iVar3_offset + 0x62f6 + param_1)
           == -0x38) {  // 200 as signed char = -56 = -0x38
            LogMessage("Not support device ...");
            CloseHandle(local_20);
            local_24++;
            continue;
        }

        // Copy inquiry data (16 bytes from offset 0x24)
        *(unsigned int*) (iVar3_offset + 0x62e3 + param_1) =
            *(unsigned int*) ((int) local_1c + 0x24);
        *(unsigned int*) (iVar3_offset + 0x62e7 + param_1) =
            *(unsigned int*) ((int) local_1c + 0x28);
        *(unsigned int*) (iVar3_offset + 0x62eb + param_1) =
            *(unsigned int*) ((int) local_1c + 0x2c);
        *(unsigned int*) (iVar3_offset + 0x62ef + param_1) =
            *(unsigned int*) ((int) local_1c + 0x30);

        // Call device support check - DAT_004ad684 function pointer
        char supportResult = CallSDKDeviceSupport(local_28, local_20);
        *(char*) (iVar3_offset + 0x62f7 + param_1) = supportResult;

        if(supportResult != '\0') {
            // Get LUN index - DAT_004ad658 function pointer
            unsigned char local_15 = 0xFF;
            int lunResult = CallSDKGetLunIndex(&local_15, local_28, local_20);
            if(lunResult == 0) {
                LogMessage("Can't get volume's Lun index ...");
            } else {
                *(unsigned char*) (iVar3_offset + 0x62e1 + param_1) = local_15;
            }

            // Get device ID - DAT_004ad660 function pointer
            unsigned char local_16 = 0xFF;
            int deviceResult = CallSDKGetDeviceID(&local_16, local_28, local_20);
            if(deviceResult == 0) {
                LogMessage("Can't get volume's DeviceID ...");
            } else {
                *(unsigned char*) (iVar3_offset + 0x62e0 + param_1) = local_16;
                if(local_16 != 0xFF) {
                    *(unsigned char*) (local_16 + 0x8a3 + param_1) = 1;
                }
            }
        }

        // Process vendor string (8 bytes from offset 8)
        for(int iVar4 = 0; iVar4 < 8; iVar4++) {
            char cVar1 = *(char*) (iVar4 + 8 + (int) local_1c);
            if(cVar1 == '\0') {
                *(unsigned char*) (iVar3_offset + iVar4 + 0x62a7 + param_1) = 0x20;  // Space
            } else {
                *(char*) (iVar3_offset + iVar4 + 0x62a7 + param_1) = cVar1;
            }
        }

        // Process product string (16 bytes from offset 0x10)
        for(int iVar4 = 0; iVar4 < 0x10; iVar4++) {
            char cVar1 = *(char*) (iVar4 + 0x10 + (int) local_1c);
            if(cVar1 == '\0') {
                *(unsigned char*) (iVar3_offset + iVar4 + 0x62b0 + param_1) = 0x20;  // Space
            } else {
                *(char*) (iVar3_offset + iVar4 + 0x62b0 + param_1) = cVar1;
            }
        }

        // Close and reopen handle - FUN_00408630
        CloseAndReopenDriveHandle(local_2c);

        // Get drive type
        UINT driveType = GetDriveTypeA(physicalDrivePaths[local_24]);
        *(UINT*) (iVar3_offset + 0x62c1 + param_1) = driveType;

        // Increment device counter and continue
        local_2c++;
        local_24++;
    }
    ...
    // Return detected device count (masked to 8 bits)
    return local_2c & 0xFF;
}

// Helper functions for OpenDriveHandleAgain - based on Ghidra MCP analysis

char iTEUFDrs::OpenPhysicalDriveByIndex(int deviceIndex) {
    // Based on FUN_00408580 decompilation at 0x00408580
    if(deviceIndex < 0 || deviceIndex >= MAX_VOLUMES)
        return '\0';

    // Calculate structure offset (0x57 bytes per device)
    int structOffset = deviceIndex * 0x57;

    // Get drive index from controller data at offset 0x62a2
    UINT driveIndex = (UINT) * (BYTE*) (structOffset + 0x62a2 + (int) this);

    // Physical drive paths - EXACT from Ghidra
    const char* physicalDrivePaths[8] = { "\\\\.\\PhysicalDrive1", "\\\\.\\PhysicalDrive2",
                                          "\\\\.\\PhysicalDrive3", "\\\\.\\PhysicalDrive4",
                                          "\\\\.\\PhysicalDrive5", "\\\\.\\PhysicalDrive6",
                                          "\\\\.\\PhysicalDrive7", "\\\\.\\PhysicalDrive8" };
    ...
    // Open with EXACT parameters from Ghidra: 0xc0000000 = GENERIC_READ | GENERIC_WRITE
    HANDLE hDevice = CreateFileA(
        physicalDrivePaths[driveIndex],
        0xC0000000,                          // EXACT from decompilation
        FILE_SHARE_READ | FILE_SHARE_WRITE,  // dwShareMode = 3
        nullptr,                             // lpSecurityAttributes = 0
        OPEN_EXISTING,                       // dwCreationDisposition = 3
        0,                                   // dwFlagsAndAttributes = 0
        nullptr                              // hTemplateFile = 0
    );

    if(hDevice == INVALID_HANDLE_VALUE) {
        DWORD error = GetLastError();
        LogMessage("Vol=%X, ERROR=%d, can't get device handle", driveIndex, error);
        return '\0';
    }

    // Store handle at offset 0x62a3
    *(HANDLE*) (structOffset + (int) this + 0x62a3) = hDevice;
    return '\1';  // Success
}

int iTEUFDrs::GetControllerDataBase() {
    // Return base address for controller data access
    // This would typically be the instance base + offset
    return (int) this;
}

int iTEUFDrs::CallSDKInquiry(void* buffer, HANDLE deviceHandle) {
    // Call SDK inquiry function via DAT_004ad520 function pointer
    if(! m_pVDR_GetDeviceInquiry)
        return 0;
    return m_pVDR_GetDeviceInquiry(buffer, deviceHandle);
}

void iTEUFDrs::FormatInquiryString(uchar** resultStr, int sourceOffset) {
    // Based on FUN_004077b0 call in decompilation
    // This formats the inquiry string from the source buffer
    if(! resultStr || ! *resultStr)
        return;

    // Copy and format the inquiry data
    char* sourceData = (char*) sourceOffset;
    if(sourceData) {
        // Update the string pointer (simplified version)
        // In actual implementation this would handle string allocation/formatting
        *resultStr = (uchar*) sourceData;
    }
}

uchar* iTEUFDrs::FindSubstring(uchar* haystack, const char* needle) {
    // Based on __mbsstr calls in decompilation
    if(! haystack || ! needle)
        return nullptr;

    // Use standard string search
    char* result = strstr((char*) haystack, needle);
    return (uchar*) result;
}

char iTEUFDrs::CallSDKDeviceSupport(void* buffer, HANDLE deviceHandle) {
    // Call SDK device support check via DAT_004ad684 function pointer
    if(! m_pVDR_CheckDeviceSupport)
        return '\0';
    return m_pVDR_CheckDeviceSupport(buffer, deviceHandle) ? '\1' : '\0';
}

int iTEUFDrs::CallSDKGetLunIndex(unsigned char* lunIndex, void* buffer, HANDLE deviceHandle) {
    // Call SDK LUN index function via DAT_004ad658 function pointer
    if(! m_pVDR_GetLunIndex)
        return 0;
    return m_pVDR_GetLunIndex(lunIndex, buffer, deviceHandle);
}

int iTEUFDrs::CallSDKGetDeviceID(unsigned char* deviceId, void* buffer, HANDLE deviceHandle) {
    // Call SDK device ID function via DAT_004ad660 function pointer
    if(! m_pVDR_GetDeviceID)
        return 0;
    return m_pVDR_GetDeviceID(deviceId, buffer, deviceHandle);
}

void iTEUFDrs::CloseAndReopenDriveHandle(int deviceIndex) {
    // Based on FUN_00408630 call in decompilation
    if(deviceIndex < 0 || deviceIndex >= MAX_VOLUMES)
        return;

    // Calculate structure offset
    int structOffset = deviceIndex * 0x57;
    HANDLE* pHandle = (HANDLE*) (structOffset + (int) this + 0x62a3);
    ...
    // Reopen would happen in subsequent calls if needed
}

void iTEUFDrs::CloseDriveHandle(int driveIndex) {
    if(driveIndex < 0 || driveIndex >= MAX_VOLUMES)
        return;
...
UINT iTEUFDrs::InitializeParaValue() {
    try {
        ...
        OutputLogMessage(
            L"InitializeParaValue: Successfully initialized device and controller structures");
        return 1;

    } catch(const std::exception& e) {
        OutputLogMessage(
            L"InitializeParaValue: Exception: "
...
UINT iTEUFDrs::VolumePairController() {
    try {
        ...
        for(int device_idx = 0; device_idx < device_count; device_idx++) {
            ...
            for(int search_idx = device_idx + 1; search_idx < device_count && devices_in_group < 4;
                search_idx++) {
                if(used_devices[search_idx] == 0
                   && device_array[search_idx].controller_type == controller_type) {
                    controller_pairs[controller_count].device_indices[devices_in_group] =
                        search_idx;
            ...
            OutputLogMessage(
                L"VolumePairController: Created controller group "
                + std::to_wstring(controller_count) + L" with " + std::to_wstring(devices_in_group)
                + L" devices");
        ...
        OutputLogMessage(
            L"VolumePairController: Created " + std::to_wstring(controller_count)
            + L" controller groups");
        return 1;

    } catch(const std::exception& e) {
        OutputLogMessage(
            L"VolumePairController: Exception: "
            + std::wstring(e.what(), e.what() + strlen(e.what())));
        return 0;
    }
}
...
UINT iTEUFDrs::DeviceManagementWorkflow() {
    try {
        ...
        for(int controller_idx = 0; controller_idx < controller_count; controller_idx++) {
            ...
            OutputLogMessage(
                L"DeviceManagementWorkflow: Processing controller "
        ...
        OutputLogMessage(
            L"DeviceManagementWorkflow: Exception: "
            + std::wstring(e.what(), e.what() + strlen(e.what())));
        return 0;
    }
}
