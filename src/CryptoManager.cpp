...
BOOL CryptoManager::SystemFunction036(void* buffer, ULONG length) {
    ...
    if (!hAdvapi32) {
        ...
        pSystemFunction036 = (BOOLEAN(WINAPI*)(void*, ULONG))
...
void* CryptoManager::SecureAlloc(SIZE_T size, const char* purpose) {
    ...
    if (!ptr) {
        LogError("CryptoManager::SecureAlloc: Failed to allocate %zu bytes for %s", 
    ...
    LogMessage("CryptoManager::SecureAlloc: Allocated %zu bytes for %s at %p", 
               size, purpose ? purpose : "unknown", userPtr);
...
BOOL CryptoManager::ValidateHashAlgorithms() {
    ...
    for (size_t i = 0; i < g_hashTestVectorCount; i++) {
        ...
        if (calculated != expected) {
            LogError("CryptoManager::ValidateHashAlgorithms: Hash test failed for '%s' - "
                     "calculated=0x%08X, expected=0x%08X", 
                     g_hashTestVectors[i].input, calculated, expected);
...
BOOL CryptoManager::VariantToString(VARIANT* variant, char* buffer, SIZE_T bufferSize) {
    ...
    switch (variant->vt) {
        ...
        case VT_I1:
            _snprintf_s(buffer, bufferSize, _TRUNCATE, "%d", variant->cVal);
            break;
            
        case VT_UI1:
            _snprintf_s(buffer, bufferSize, _TRUNCATE, "%u", variant->bVal);
            break;
            
        case VT_I2:
            _snprintf_s(buffer, bufferSize, _TRUNCATE, "%d", variant->iVal);
            break;
            
        case VT_UI2:
            _snprintf_s(buffer, bufferSize, _TRUNCATE, "%u", variant->uiVal);
            break;
            
        case VT_I4:
            _snprintf_s(buffer, bufferSize, _TRUNCATE, "%ld", variant->lVal);
            break;
            
        case VT_UI4:
            _snprintf_s(buffer, bufferSize, _TRUNCATE, "%lu", variant->ulVal);
            break;
            
        case VT_BSTR:
            if (variant->bstrVal) {
                // Convert BSTR to char string
                int len = WideCharToMultiByte(CP_ACP, 0, variant->bstrVal, -1, 
        ...
        case VT_BOOL:
            strcpy_s(buffer, bufferSize, variant->boolVal ? "TRUE" : "FALSE");
            break;
            
        default:
            // For unsupported types, convert the raw data
            _snprintf_s(buffer, bufferSize, _TRUNCATE, "VT_%d_DATA", variant->vt);
            LogWarning("CryptoManager::VariantToString: Unsupported variant type %d", 
                       variant->vt);
            break;
    }
