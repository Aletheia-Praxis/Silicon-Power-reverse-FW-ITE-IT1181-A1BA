...
CDeviceSelectDialog::CDeviceSelectDialog(CWnd* pParent)
    : CDialog(IDD_DEVICE_SELECT, pParent)
{
    m_selectedDevice = _T("");
}
...
// Get the device path
BOOL CDeviceSelectDialog::GetDevicePath(HDEVINFO hDevInfo, PSP_DEVINFO_DATA pDeviceInfoData, 
                                       LPTSTR pDevicePath, DWORD devicePathSize)
{
    // Get the device interface
    SP_DEVICE_INTERFACE_DATA deviceInterfaceData;
    deviceInterfaceData.cbSize = sizeof(SP_DEVICE_INTERFACE_DATA);
    
    if (!SetupDiEnumDeviceInterfaces(hDevInfo, pDeviceInfoData, &GUID_DEVINTERFACE_USB_DEVICE, 
    ...
    if (SetupDiGetDeviceInterfaceDetail(hDevInfo, &deviceInterfaceData, pDeviceInterfaceDetailData, 
