...
BOOL HexStringToBytes(LPCSTR hexString, BYTE* pBytes, DWORD* pByteCount) {
    if(! hexString || ! pBytes || ! pByteCount) {
        return FALSE;
    }

    DWORD hexLength = (DWORD) strlen(hexString);
    if(hexLength % 2 != 0) {
        return FALSE;
    }

    *pByteCount = hexLength / 2;

    for(DWORD i = 0; i < *pByteCount; i++) {
        char hexByte[3] = { hexString[i * 2], hexString[i * 2 + 1], 0 };
        pBytes[i] = (BYTE) strtol(hexByte, NULL, 16);
    }
...
DWORD CalculateCRC32(const BYTE* pData, DWORD dataSize) {
    ...
    for(DWORD i = 0; i < dataSize; i++) {
        crc = crc32Table[(crc ^ pData[i]) & 0xFF] ^ (crc >> 8);
    }

    return crc ^ 0xFFFFFFFF;
}

// Firmware file verification function
BOOL ValidateFirmwareFile(LPCSTR filePath) {
    ...
    if(fileSize == INVALID_FILE_SIZE || fileSize < ITE_HEADER_SIZE) {
        return FALSE;
    }

    if(fileSize > ITE_MAX_FIRMWARE_SIZE) {
        return FALSE;
    }
...
const char* FormatCapacityLabelMB(uint32_t valueMB) {
    ...
    const char* candidates[] = { "128MB", "256MB", "512MB", "1GB",   "2GB",   "4GB",   "8GB",
                                 "16GB",  "32GB",  "64GB",  "128GB", "256GB", "512GB", "1TB" };
    const uint32_t stepsMB[] = { 128,   256,   512,   1024,   2048,   4096,   8192,
...
uint32_t swap32_mixed(uint32_t value) {
    ...
    return (value << 24) | (value >> 24) | ((value & 0x0000FF00) << 8)
           | ((value & 0x00FF0000) >> 8);
}
